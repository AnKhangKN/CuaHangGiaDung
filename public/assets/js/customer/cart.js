$(document).ready(function () {
  ...
  function updateTotalAllPrice() {
    ...
    $(".cart_contents_table").each(function () {
      ...
      var cleanPrice =
        parseFloat(totalUnitText.replace(/\./g, "").replace(",", ".")) || 0;
  ...
  $(".cart-table-number").on("input", function () {
    ...
    $.ajax({
      type: "POST",
      url: "/CuaHangGiaDung/app/controllers/customer/update_cart.php", // Đảm bảo URL đúng
      data: {
        action: "update",
        productId: productId,
        size: size,
  ...
  $(".cart-table-remove").click(function (e) {
    ...
    $.ajax({
      type: "POST",
      url: "/CuaHangGiaDung/app/controllers/customer/delete_cart_item.php", // Đảm bảo URL đúng
      data: {
        action: "remove",
        productId: productId,
        size: size,
  ...
  $(".cart-table-plus").click(function (e) {
    ...
    $.ajax({
      type: "POST",
      url: "/CuaHangGiaDung/app/controllers/customer/update_cart.php",
      data: {
        action: "update",
        productId: productId,
        size: size,
  ...
  $("#pay_page").click(function (e) {
    ...
    var cleanPrice =
      parseFloat(totalPrice.replace(/\./g, "").replace(",", ".")) || 0;
    if (cleanPrice > 0) {
