...
const upUserOption = document.getElementsByClassName(
...
document.addEventListener("click", function (event) {
  ...
  if (
    !userOptionMark.contains(event.target) &&
    !userOption.contains(event.target)
  ) {
...
$(document).ready(function () {
  $("#search_box_input").on("input", function () {
    ...
    if (query.length > 0) {
      ...
      $.ajax({
        url: "/CuaHangGiaDung/app/controllers/customer/search_products.php", // Đường dẫn tới file xử lý tìm kiếm
        method: "GET",
        data: { query: query },
