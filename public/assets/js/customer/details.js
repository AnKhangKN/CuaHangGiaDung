...
function createProductRecommend(
  productsListIdRecommend,
  prevBtnIdRecommend,
  nextBtnIdRecommend
) {
  const productsDetailsListRecommend = document.getElementById(
    productsListIdRecommend
  );
  ...
  function updateButtons() {
    ...
    nextBtnDetailsRecommend.disabled =
      currentIndex >= totalProductsRecommend - visibleProducts; // Vô hiệu hóa nút 'Tiếp theo' nếu không còn sản phẩm để hiển thị
  }
  ...
  nextBtnDetailsRecommend.addEventListener("click", () => {
    if (currentIndex < totalProductsRecommend - visibleProducts) {
      // Chỉ cuộn nếu không ở sản phẩm cuối
      currentIndex++;
  ...
  prevBtnDetailsRecommend.addEventListener("click", () => {
    if (currentIndex > 0) {
      // Chỉ cuộn nếu không ở sản phẩm đầu
      currentIndex--;
      scrollToIndex(currentIndex);
      updateButtons();
    }
...
$(document).ready(function () {
  ...
  $(".products_details_info_add_products_minus").click(function (e) {
    ...
    const inputField = productInfo.find(
      ".products_details_info_add_products_input"
    );
  ...
  $('.products_details_info_size_group_item input[type="checkbox"]').change(
    function () {
      if ($(this).is(":checked")) {
        $('.products_details_info_size_group_item input[type="checkbox"]')
          .not(this)
          .prop("checked", false);
      }
    }
  ...
  $('.products_details_info_color_item input[type="checkbox"]').change(
    function () {
      if ($(this).is(":checked")) {
        $('.products_details_info_color_item input[type="checkbox"]')
          .not(this)
          .prop("checked", false);
      }
    }
  ...
  $(
    ".products_details_info_size_group_item_input, .products_details_info_color_item_input"
  ).change(function () {
    const productInfo = $(this).closest(".product_info_to_cart");

    let size = productInfo
      .find(".products_details_info_size_group_item_input:checked")
      .val();
    if (!size) {
      size = "Không có kích thước";
    }

    let color = productInfo
      .find(".products_details_info_color_item_input:checked")
      .val();
    ...
    $.ajax({
      type: "POST",
      url: "/CuaHangGiaDung/app/controllers/customer/ProductAmount.php",
      data: {
        action: "select",
        size: size,
        color: color,
  ...
  $(".add_cart").click(function (e) {
    ...
    let soluong = parseInt(
      productInfo.find(".products_details_info_add_products_input").val()
    );
    ...
    if (soluong > 0 && amount > 0) {
      ...
      $.ajax({
        type: "POST",
        url: "/CuaHangGiaDung/app/controllers/customer/add_to_cart.php",
        data: {
