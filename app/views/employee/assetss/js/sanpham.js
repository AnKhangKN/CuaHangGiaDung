...
$(document).ready(function () {
  ...
  $(document).on("click", ".remove_cart", function () {
    ...
    const idChiTietSanPham = container_cart
      .find(".idChiTietSanPham_buy")
      .text();
...
$(document).ready(function () {
  ...
  function dsDonHangCoIdKH() {
    ...
    const tongtien = parseFloat(
      container_payment.find("#tong_don_gia").text().trim()
    ); // Convert to number

    if (!idKhachHang || !idNhanVien || isNaN(tongtien)) {
      alert(
        "Vui lòng kiểm tra lại giá trị của 'idKhachHang', 'idNhanVien', và 'tongtien'."
      );
      return;
    }
    ...
    $(".SanPham_buy").each(function () {
      ...
      const idChiTietSanPham = lstProduct
        .find(".idChiTietSanPham_buy")
        .text()
        .trim();
